...
public abstract class AbstractGraph implements Graph {
    ...
    public void loadEdgesFromFile(Scanner scan) throws IOException {
       ...
       try {
            ...
            while (scan.hasNextLine()) {
                ...
                if (parts.length == 2)
                    insert(new Edge(Integer.parseInt(parts[0]), Integer.parseInt(parts[1]), 1.0));
                else if (parts.length == 3)
                    insert(new Edge(Integer.parseInt(parts[0]), Integer.parseInt(parts[1]), Double.parseDouble(parts[2])));
     ...
     *             is to be created
     * @throws IOException if type is neither "Matrix"
     *                                  nor "List"
